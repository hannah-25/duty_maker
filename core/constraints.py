...
class ScheduleModel:
    ...
    def _rule_staffing_range(self):
        ...
        for day in self.current_days:
            ...
            self._enforce(self.model.Add(d_total >= req.D.minimum), lit)
            self._enforce(self.model.Add(d_total <= req.D.maximum), lit)
    ...
    def _soft_s_daily_target(self):
        """S 사용 병동은 가능한 날마다 S 1명을 선호한다.

        S는 D/E/N과 별도 근무이지만 이전의 "S 사용 최소화" 벌점과 함께 두면
        S가 필요하지 않은 모든 경우에 배정이 0명으로 수렴했다.
        인력·자격·다른 하드 제약이 허용하지 않으면 벌점만 발생하며 생성 자체는 막지 않는다.
        """
        if not self.settings.get("use_s_shift", True):
            return
        for day in self.current_days:
            s_total = sum(self.val(nurse.name, day, ShiftType.S) for nurse in self.nurses)
            missing = self.model.NewIntVar(0, 1, f"s_daily_missing_{day.isoformat()}")
            self.model.Add(missing >= 1 - s_total)
            self.penalties.append(("s_daily_target", WEIGHT_S_DAILY_TARGET, missing))
