...
WEIGHT_WEEKEND_CHARGE = 30  # 주말 데이 차지 2명 미만
...
DEFAULT_WARD_SETTINGS = {
    "senior_night_exactly_one": True,  # 나이트에 시니어(책임급) 정확히 1명
    "weekday_day_charge_min": 2,  # 평일 데이 차지 최소 인원 (0이면 미적용)
}
...
class ScheduleModel:
    ...
    def _rule_weekday_charge_minimum(self):
        """평일 D에는 차지 가능 인력을 최소 N명 배치한다 (병동 설정, 기본 2, 0이면 미적용).

        주말은 제외한다 — 주말은 인원 자체가 적어(D 하한 2) 차지 2명이 과도하고,
        주말 D에도 차지 최소 1명은 _rule_charge_placement가 이미 보장한다.
        """
        minimum = self.settings["weekday_day_charge_min"]
        if minimum <= 0:
            return
    ...
    def _rule_senior_same_shift_cap(self, maximum: int = 2):
        """시니어(책임급) 데이 최대 2명, 나이트 정확히 1명. 이브닝은 소프트(_soft_senior_e_cap).

        나이트 '정확히 1명'은 병동 설정(senior_night_exactly_one)으로 끌 수 있다 —
        나이트 전담이 전부 책임급인 병동에서는 이 제약이 근무표를 불가능하게 만든다.
        """
        ...
        for day in self.current_days:
            ...
            if self.settings["senior_night_exactly_one"]:
    ...
    def add_tier2_soft_constraints(self, duty_requests: list[DutyRequest] | None = None):
        ...
        self._soft_weekend_charge_minimum()
    ...
    def _soft_weekend_charge_minimum(self, minimum: int = 2):
        """주말 데이에도 차지 가능자 2명을 선호 (미달분에 벌점). 최소 1명은 charge_placement가 보장."""
        for day in self.current_days:
            if day.weekday() < 5:
                continue
            charge_d = sum(self.val(n.name, day, ShiftType.D) for n in self.nurses if n.can_charge)
            shortfall = self.model.NewIntVar(0, minimum, f"weekend_charge_{day.isoformat()}")
            self.model.Add(shortfall >= minimum - charge_d)
            self.penalties.append(("weekend_charge", WEIGHT_WEEKEND_CHARGE, shortfall))
